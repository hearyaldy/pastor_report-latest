...
class ImprovedAdminDashboardOptimized extends StatefulWidget {
  const ImprovedAdminDashboardOptimized({super.key});

  @override
  State<ImprovedAdminDashboardOptimized> createState() =>
      _ImprovedAdminDashboardOptimizedState();
}

class _ImprovedAdminDashboardOptimizedState
    extends State<ImprovedAdminDashboardOptimized> {
  ...
  // Cached statistics to prevent redundant API calls
  int _totalUsers = 0;
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      // Check if data was loaded recently (within 30 seconds) to prevent excessive API calls
    ...
    try {
      ...
      if (userMission != null && userMission.isNotEmpty) {
        ...
        if (user?.userRole == UserRole.districtPastor) {
          ...
          final futures = districts.map(
              (district) => _churchService.getChurchesByDistrict(district.id));
          final allChurches = await Future.wait(futures);
          _totalChurches =
              allChurches.fold(0, (sum, churches) => sum + churches.length);

          debugPrint(
              '📊 Loaded $_totalChurches churches for mission: $userMission');
        }
